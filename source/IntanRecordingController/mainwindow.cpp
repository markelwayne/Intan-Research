...
void MainWindow::createLayout()
{
    ...
    runContinuousImpedanceTestButton = new QPushButton(tr("Run Continuous Impedance Measurement"));
    runImpedanceTestButton->setEnabled(false);
    runContinuousImpedanceTestButton->setEnabled(false);
    ...
    connect(runImpedanceTestButton, SIGNAL(clicked()),
     ...
     connect(runContinuousImpedanceTestButton, SIGNAL(clicked()),
            this, SLOT(runContinuousImpedanceMeasurement()));


    continuousImpedanceCheckBox = new QCheckBox(tr("Keep Running Impedance Test"));
    ...
    QHBoxLayout *runContinuousImpedanceTestLayout = new QHBoxLayout();

    runImpedanceTestLayout->addWidget(runImpedanceTestButton);
    runImpedanceTestLayout->addStretch(1);

    runContinuousImpedanceTestLayout->addWidget(runContinuousImpedanceTestButton);
    runContinuousImpedanceTestLayout->addStretch(1);
    ...
    impedanceLayout->addLayout(runContinuousImpedanceTestLayout);
    impedanceLayout->addWidget(continuousImpedanceCheckBox);
...
void MainWindow::updateImpedanceFrequency()
{
    ...
    runContinuousImpedanceTestButton->setEnabled(impedanceFreqValid);
...
// Execute a continuous electrode impedance measurement procedure.
void MainWindow::runContinuousImpedanceMeasurement()
{
    ...
    statusBar()->showMessage("Measuring continuous electrode impedances...");
    ...
    while(1)
    {
        if(continuousImpedanceCheckBox->isChecked()==false)
        {
            break;
        }
        for (stream = 0; stream < evalBoard->getNumEnabledDataStreams(); ++stream) {
