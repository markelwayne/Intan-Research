...
// class is to read from a queue of Rhd2000DataBlockUsb3 objects and scale
...
void SignalProcessor::allocateMemory(int numStreams)
{
    ...
    allocateDoubleArray3D(auxChannel, numStreams, 3, (SAMPLES_PER_DATA_BLOCK / 4) * MAX_NUM_BLOCKS);
    allocateDoubleArray2D(supplyVoltage, numStreams, MAX_NUM_BLOCKS);
    ...
    allocateDoubleArray2D(tempRawHistory, numStreams, MAX_NUM_BLOCKS);
    tempHistoryReset(4);
}
...
void SignalProcessor::openSignalTypeFiles(bool saveTtlOut)
{
    ...
    auxInputStream = nullptr;
    supplyStream = nullptr;
...
// Reads numBlocks blocks of raw USB data stored in a queue of Rhd2000DataBlockUsb3
...
int SignalProcessor::loadAmplifierData(queue<Rhd2000DataBlockUsb3> &dataQueue,
                                       int numBlocks, bool lookForTrigger, int triggerChannel,
                                       int triggerPolarity, int &triggerTimeIndex, bool addToBuffer, queue<Rhd2000DataBlockUsb3> &bufferQueue,
                                       bool saveToDisk, QDataStream &out, SaveFormat format,
                                       bool saveTtlOut, int timestampOffset, ReferenceSource referenceSource)
{
...
//        }
        ...
        // We are done with this Rhd2000DataBlockUsb3 object; remove it from dataQueue
        dataQueue.pop();
    }
...
int SignalProcessor::saveBufferedData(queue<Rhd2000DataBlockUsb3> &bufferQueue, QDataStream &out, SaveFormat format,
                                      bool saveTtlOut, int timestampOffset)
{
    ...
    case SaveFormatIntan:
        while (bufferQueue.empty() == false) {
            ...
            // We are done with this Rhd2000DataBlockUsb3 object; remove it from bufferQueue
            bufferQueue.pop();
        }
        break;
    case SaveFormatFilePerSignalType:
...
int SignalProcessor::loadSyntheticData(int numBlocks, double sampleRate,
                                       ...
                                       SaveFormat format, bool saveTtlOut,
            ...
            for (block = 0; block < numBlocks; ++block) {
                ...
                for (i = 0; i < saveListAuxInput.size(); ++i) {
                    for (t = 0; t < (SAMPLES_PER_DATA_BLOCK / 4); ++t) {
                        out << (quint16)
                               (auxChannel[saveListAuxInput.at(i)->boardStream][saveListAuxInput.at(i)->chipChannel][(SAMPLES_PER_DATA_BLOCK / 4) * block + t] / 0.0000374);
                        ++numWordsWritten;
                    }
                }
                // Save supply voltage data
                ...
                for (t = 0; t < SAMPLES_PER_DATA_BLOCK; ++t) {
                    ...
                    for (i = 0; i < saveListAuxInput.size(); ++i) {
                        *(auxInputStream) << (quint16)
                            (auxChannel[saveListAuxInput.at(i)->boardStream][saveListAuxInput.at(i)->chipChannel][(SAMPLES_PER_DATA_BLOCK /4) * block + tAux] / 0.0000374);
                        ++numWordsWritten;
                    }
                    // Save supply voltage data
                    for (i = 0; i < saveListSupplyVoltage.size(); ++i) {
...
int SignalProcessor::bytesPerBlock(SaveFormat saveFormat, bool saveTtlOut)
{
    ...
    if (saveFormat == SaveFormatIntan) {
        ...
        bytes += 2 * (SAMPLES_PER_DATA_BLOCK / SAMPLES_PER_DATA_BLOCK) * saveListSupplyVoltage.size();
    } else {
