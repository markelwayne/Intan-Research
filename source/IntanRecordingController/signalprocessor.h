...
// The maximum number of Rhd2000DataBlockUsb3 objects needed during normal data acquisition.
...
public:
    ...
    int loadAmplifierData(queue<Rhd2000DataBlockUsb3> &dataQueue, int numBlocks,
                          bool lookForTrigger, int triggerChannel, int triggerPolarity,
                          int &triggerIndex, bool addToBuffer,
                          queue<Rhd2000DataBlockUsb3> &bufferQueue, bool saveToDisk, QDataStream &out,
                          SaveFormat format, bool saveTtlOut, int timestampOffset, ReferenceSource referenceSource);
    int loadSyntheticData(int numBlocks, double sampleRate, bool saveToDisk,
                          QDataStream &out, SaveFormat format, bool saveTtlOut, ReferenceSource &referenceSource);
    int saveBufferedData(queue<Rhd2000DataBlockUsb3> &bufferQueue, QDataStream &out, SaveFormat format, bool saveTtlOut, int timestampOffset);
...
private:
    ...
    char dataStreamBuffer[2 * MAX_NUM_DATA_STREAMS * CHANNELS_PER_STREAM * SAMPLES_PER_DATA_BLOCK];
