...
SignalSources::SignalSources(int numPorts)
{
    ...
    // We have up to eight SPI ports, 8 ADC inputs, and 16 digital inputs and outputs.
    signalPort.resize(numPorts + 3);
    ...
    signalPort[numPorts + 1].name = "Digital In Ports";
    signalPort[numPorts + 2].name = "Digital Out Ports";

    signalPort[numPorts + 0].prefix = "ANALOG-IN";
    signalPort[numPorts + 1].prefix = "DIGITAL-IN";
    signalPort[numPorts + 2].prefix = "DIGITAL-OUT";

    signalPort[numPorts + 0].enabled = true;   // Board analog inputs are always enabled
    signalPort[numPorts + 1].enabled = true;   // Board digital inputs are always enabled
    signalPort[numPorts + 2].enabled = true;
    ...
    for (int channel = 0; channel < 16; ++channel) {
        signalPort[numPorts + 1].addBoardDigInChannel(channel);
        signalPort[numPorts + 1].channel[channel].enabled = false;
    }
    // Add 16 board digital output signals
    for (int channel = 0; channel < 16; ++channel) {
        signalPort[numPorts + 2].addBoardDigOutChannel(channel);
        signalPort[numPorts + 2].channel[channel].enabled = true;
    }
    // Amplifier channels on SPI ports A-H are added later, if amplifier
