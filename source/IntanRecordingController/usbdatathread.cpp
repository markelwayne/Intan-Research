...
UsbDataThread::UsbDataThread(Rhd2000EvalBoardUsb3* board_, DataStreamFifo* usbFifo_, QObject *parent) :
    QThread(parent),
    board(board_),
    usbFifo(usbFifo_)
{
    ...
    unsigned int bufferSize = BUFFER_SIZE_IN_BLOCKS * 2 * Rhd2000DataBlockUsb3::calculateDataBlockSizeInWords(MAX_NUM_DATA_STREAMS);
